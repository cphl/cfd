...
# Step 4
def diffusion_1(nx, x_max, nt, u0, nu=0.3, sigma=0.2):
    """Propagates the initial condition to give the diffuse solution
    Forward difference in time, central difference for 2nd order spatial deriv
    nx -- number of points on spatial grid
    x_max -- spatial range goes from 0 to max_x
    dt -- size of timestep interval
    nt -- number of timesteps to take total
    ...
    """
    dx = x_max/(nx-1)  # x grid spacing
    dt = sigma*dx**2/nu  # set dt subject to CFL condition
    u = np.ones((nt, nx))  # time=rows, space=columns
    u[0, :] = u0  # set initial condition to be soln at t=0
    ...
    for t in range(1, nt):
        # internal points
        u[t, 1:-1] = u[t-1, 1:-1] - \
            nu*dt/(dx**2) * (u[t-1, 2:] - 2*u[t-1, 1:-1] + u[t-1, :-2])
            # central difference u_i+1 - 2u_i           + u_i-1
    return u
...
nx = 82  # was 41. 82, 84 gets artifacts
x_max = 2
dt = 0.025
nt = 40
dx = x_max/(nx-1)
...
# 4) Diffusion
diffusolution = diffusion_1(nx, x_max, nt, u0)
plot1d(diffusolution, 0, x_max)
# TODO weird artifacts with diffusion. Might be array shift problem